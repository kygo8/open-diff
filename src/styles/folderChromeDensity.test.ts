...
describe('folder chrome density', () => {
  it('keeps Folder/Sync/Merge session bodies and strips dense toward capture', () => {
    ...
    expect(folderView).toMatch(/\.path-side-footer\s*\{[\s\S]*?font-size:\s*6px/)
    expect(folderView).toMatch(/\.path-side-footer\s*\{[\s\S]*?min-height:\s*6px/)
    expect(folderView).toMatch(/\.path-side-footer\s*\{[\s\S]*?line-height:\s*6px/)
    expect(folderView).toMatch(/\.path-pair input\s*\{[\s\S]*?height:\s*16px/)
...
it('surfaces display-filters instead of CSS-hiding the Filters strip', () => {
  ...
  expect(folderView).toMatch(/data-filters-density="capture-pass11"/)
  ...
  expect(syncView).toMatch(/data-filters-density="capture-pass11"/)
  ...
  expect(mergeView).toMatch(/data-filters-density="capture-pass11"/)
