...
it('surfaces display-filters instead of CSS-hiding the Filters strip', () => {
  ...
  expect(css).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*26px/)
  expect(css).toMatch(/\.folder-filter-pattern\s*\{[\s\S]*?height:\s*18px/)
  expect(css).toMatch(/\.folder-compare-view \.folder-filter-chrome,[\s\S]*?min-height:\s*26px/)
  expect(css).toMatch(/\.folder-compare-view \.display-filters,[\s\S]*?gap:\s*1px 3px/)
  expect(css).toMatch(/\.folder-compare-view \.display-filters,[\s\S]*?min-height:\s*18px/)
  ...
  expect(folderView).toMatch(/data-filters-density="capture-pass4"/)
  expect(folderView).toMatch(/\.folder-filter-chrome\s*\{[\s\S]*?min-height:\s*26px/)
  expect(folderView).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*26px/)
  expect(folderView).toMatch(/\.folder-filter-pattern\s*\{[\s\S]*?height:\s*18px/)
  expect(folderView).toMatch(/:size="12"/)
  ...
  expect(syncView).toMatch(/data-filters-density="capture-pass4"/)
  expect(syncView).toMatch(/\.folder-filter-chrome\s*\{[\s\S]*?min-height:\s*26px/)
  expect(syncView).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*26px/)
  ...
  expect(mergeView).toMatch(/data-filters-density="capture-pass4"/)
  expect(mergeView).toMatch(/\.folder-filter-chrome\s*\{[\s\S]*?min-height:\s*26px/)
  expect(mergeView).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*26px/)
})
