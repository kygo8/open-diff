...
it('surfaces display-filters instead of CSS-hiding the Filters strip', () => {
  ...
  expect(css).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*24px/)
  expect(css).toMatch(/\.folder-filter-pattern\s*\{[\s\S]*?height:\s*16px/)
  expect(css).toMatch(/\.folder-compare-view \.folder-filter-chrome,[\s\S]*?min-height:\s*24px/)
  expect(css).toMatch(/\.folder-compare-view \.display-filters,[\s\S]*?gap:\s*1px 3px/)
  expect(css).toMatch(/\.folder-compare-view \.display-filters,[\s\S]*?min-height:\s*16px/)
  ...
  expect(folderView).toMatch(/data-filters-density="capture-pass5"/)
  expect(folderView).toMatch(/\.folder-filter-chrome\s*\{[\s\S]*?min-height:\s*24px/)
  expect(folderView).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*24px/)
  expect(folderView).toMatch(/\.folder-filter-pattern\s*\{[\s\S]*?height:\s*16px/)
  ...
  expect(syncView).toMatch(/data-filters-density="capture-pass5"/)
  expect(syncView).toMatch(/\.folder-filter-chrome\s*\{[\s\S]*?min-height:\s*24px/)
  expect(syncView).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*24px/)
  ...
  expect(mergeView).toMatch(/data-filters-density="capture-pass5"/)
  expect(mergeView).toMatch(/\.folder-filter-chrome\s*\{[\s\S]*?min-height:\s*24px/)
  expect(mergeView).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*24px/)
})
