...
it('surfaces display-filters instead of CSS-hiding the Filters strip', () => {
  ...
  expect(css).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*32px/)
  expect(css).toMatch(/\.folder-filter-pattern\s*\{[\s\S]*?height:\s*22px/)
  expect(css).toMatch(/\.folder-compare-view \.folder-filter-chrome,[\s\S]*?min-height:\s*32px/)
  ...
  expect(folderView).toMatch(/\.folder-filter-chrome\s*\{[\s\S]*?min-height:\s*32px/)
  expect(folderView).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*32px/)
  ...
  expect(syncView).toMatch(/\.folder-filter-chrome\s*\{[\s\S]*?min-height:\s*32px/)
  ...
  expect(mergeView).toMatch(/\.folder-filter-chrome\s*\{[\s\S]*?min-height:\s*32px/)
})
