...
it('surfaces display-filters instead of CSS-hiding the Filters strip', () => {
  ...
  expect(css).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*28px/)
  expect(css).toMatch(/\.folder-filter-pattern\s*\{[\s\S]*?height:\s*20px/)
  expect(css).toMatch(/\.folder-compare-view \.folder-filter-chrome,[\s\S]*?min-height:\s*28px/)
  expect(css).toMatch(/\.folder-compare-view \.display-filters,[\s\S]*?gap:\s*2px 4px/)
  ...
  expect(folderView).toMatch(/\.folder-filter-chrome\s*\{[\s\S]*?min-height:\s*28px/)
  expect(folderView).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*28px/)
  ...
  expect(syncView).toMatch(/\.folder-filter-chrome\s*\{[\s\S]*?min-height:\s*28px/)
  expect(syncView).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*28px/)
