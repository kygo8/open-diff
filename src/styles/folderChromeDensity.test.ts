...
describe('folder chrome density', () => {
  it('keeps Folder/Sync/Merge session bodies at capture CSS scale', () => {
    ...
    expect(folderView).toMatch(/\.path-side-footer\s*\{[\s\S]*?font-size:\s*11px/)
    expect(folderView).toMatch(/\.path-side-footer\s*\{[\s\S]*?min-height:\s*20px/)
    expect(folderView).toMatch(/\.path-side-footer\s*\{[\s\S]*?line-height:\s*16px/)
