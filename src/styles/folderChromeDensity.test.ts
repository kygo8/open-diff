...
it('surfaces display-filters instead of CSS-hiding the Filters strip', () => {
  ...
  expect(css).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*22px/)
  expect(css).toMatch(/\.folder-filter-pattern\s*\{[\s\S]*?height:\s*15px/)
  expect(css).toMatch(/\.folder-compare-view \.folder-filter-chrome,[\s\S]*?min-height:\s*22px/)
  expect(css).toMatch(/\.folder-compare-view \.display-filters,[\s\S]*?gap:\s*1px 3px/)
  expect(css).toMatch(/\.folder-compare-view \.display-filters,[\s\S]*?min-height:\s*15px/)
  ...
  expect(folderView).toMatch(/data-filters-density="capture-pass6"/)
  expect(folderView).toMatch(/\.folder-filter-chrome\s*\{[\s\S]*?min-height:\s*22px/)
  expect(folderView).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*22px/)
  expect(folderView).toMatch(/\.folder-filter-pattern\s*\{[\s\S]*?height:\s*15px/)
  ...
  expect(syncView).toMatch(/data-filters-density="capture-pass6"/)
  expect(syncView).toMatch(/\.folder-filter-chrome\s*\{[\s\S]*?min-height:\s*22px/)
  expect(syncView).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*22px/)
  ...
  expect(mergeView).toMatch(/data-filters-density="capture-pass6"/)
  expect(mergeView).toMatch(/\.folder-filter-chrome\s*\{[\s\S]*?min-height:\s*22px/)
  expect(mergeView).toMatch(/\.folder-filter-strip-btn\s*\{[\s\S]*?height:\s*22px/)
})
