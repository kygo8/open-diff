...
describe('home chrome density', () => {
  it('keeps Home CTA/tree/header/footer chrome dense toward capture', () => {
    expect(homeView).toMatch(/data-home-density="capture-pass14"/)
    ...
    expect(homeView).toMatch(/\.bc-selected-actions button\s*\{[\s\S]*?height:\s*8px/)
    expect(homeView).toMatch(/\.new-session-grid\s*\{[\s\S]*?gap:\s*0 1px/)
    expect(homeView).toMatch(/\.session-card-icon\s*\{[\s\S]*?width:\s*18px/)
    expect(homeView).toMatch(/\.new-session-card\s*\{[\s\S]*?min-height:\s*26px/)
    expect(homeView).toMatch(/\.new-session-card\s*\{[\s\S]*?border-radius:\s*0/)
    expect(homeView).toMatch(/:size="12"/)
