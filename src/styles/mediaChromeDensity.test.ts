...
describe('media compare chrome density', () => {
  ...
  it('keeps Media transport/pane chrome dense toward Hex/Table/Version rhythm', () => {
    expect(css).toMatch(/\.media-compare-view \.media-summary-item[\s\S]*?padding:\s*2px 6px/)
    expect(css).toMatch(/\.media-compare-view \.media-report-panel[\s\S]*?padding:\s*2px 6px/)
    expect(css).toMatch(/\.media-compare-view \.media-rules-panel[\s\S]*?padding:\s*2px 6px/)
    ...
    expect(mediaView).toMatch(/\.media-summary-item[\s\S]*?padding:\s*2px 6px/)
    expect(mediaView).toMatch(/\.media-report-panel[\s\S]*?padding:\s*2px 6px/)
    expect(mediaView).toMatch(/\.media-rules-panel[\s\S]*?padding:\s*2px 6px/)
    expect(mediaView).toMatch(/\.media-field-row\s*\{[\s\S]*?min-height:\s*16px/)
    expect(mediaView).toMatch(/\.media-side dl div\s*\{[\s\S]*?padding:\s*2px 6px/)
