...
describe('path/status strip density', () => {
  it('keeps path bars and footers denser toward capture chrome', () => {
    ...
    expect(css).toMatch(/\.bc-path-row input\s*\{[\s\S]*?height:\s*14px/)
    expect(css).toMatch(/\.path-pair-bar\s*\{[\s\S]*?min-height:\s*22px/)
    expect(css).toMatch(/\.path-side-footer\s*\{[\s\S]*?font-size:\s*8px/)
    expect(css).toMatch(/\.path-side-footer\s*\{[\s\S]*?min-height:\s*8px/)
    expect(css).toMatch(/\.path-side-footer\s*\{[\s\S]*?line-height:\s*8px/)
  })
