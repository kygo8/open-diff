...
describe('path/status strip density', () => {
  it('keeps path bars and footers denser toward capture chrome', () => {
    expect(css).toMatch(/\.bc-path-row\s*\{[\s\S]*?min-height:\s*22px/)
    expect(css).toMatch(/\.bc-path-row input\s*\{[\s\S]*?height:\s*16px/)
    expect(css).toMatch(/\.path-pair-bar\s*\{[\s\S]*?min-height:\s*24px/)
    expect(css).toMatch(/\.path-side-footer\s*\{[\s\S]*?font-size:\s*8px/)
    expect(css).toMatch(/\.path-side-footer\s*\{[\s\S]*?min-height:\s*9px/)
    expect(css).toMatch(/\.path-side-footer\s*\{[\s\S]*?line-height:\s*9px/)
  })
  ...
  it('fuses path row corners and borders into the shell frame', () => {
    ...
    expect(css).toMatch(
      /\.app-shell-single-session \.folder-toolbar \.path-pair\s*\{[\s\S]*?min-height:\s*22px/,
    )
  })
})
