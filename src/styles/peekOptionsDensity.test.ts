...
describe('peek panel densify', () => {
  it('keeps Folder/Sync/Merge peek panels dense with tab chrome', () => {
    ...
    expect(css).toMatch(/\.peek-tab\s*\{[\s\S]*?height:\s*12px/)
    ...
    expect(css).toMatch(/\.folder-peek-panel dl > div,[\s\S]*?min-height:\s*15px/)
    expect(css).toMatch(/\.folder-peek-panel header,[\s\S]*?min-height:\s*12px/)
    expect(css).toMatch(/\.peek-tabs\s*\{[\s\S]*?min-height:\s*12px/)
    ...
    expect(folderView).toMatch(/data-peek-density="capture-pass5"/)
    ...
    expect(folderView).toMatch(/\.folder-peek-panel header\s*\{[\s\S]*?min-height:\s*12px/)
    ...
    expect(syncView).toMatch(/data-peek-density="capture-pass5"/)
    expect(syncView).toMatch(/\.folder-sync-peek-panel\s*\{[\s\S]*?padding:\s*0/)
    expect(syncView).toMatch(/\.folder-sync-peek-panel header\s*\{[\s\S]*?min-height:\s*12px/)

    expect(mergeView).toMatch(/data-testid="folder-merge-peek-tabs"/)
    expect(mergeView).toMatch(/data-testid="folder-merge-peek-dual"/)
    expect(mergeView).toMatch(/data-peek-density="capture-pass5"/)
    expect(mergeView).toMatch(/\.folder-merge-peek-panel\s*\{[\s\S]*?padding:\s*0/)
    expect(mergeView).toMatch(/\.folder-merge-peek-panel header\s*\{[\s\S]*?min-height:\s*12px/)
  })
})
