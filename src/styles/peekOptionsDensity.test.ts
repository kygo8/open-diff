...
describe('peek panel densify', () => {
  it('keeps Folder/Sync/Merge peek panels dense with tab chrome', () => {
    ...
    expect(css).toMatch(/\.peek-tab\s*\{[\s\S]*?height:\s*13px/)
    expect(css).toMatch(/\.peek-tab\s*\{[\s\S]*?font-size:\s*10px/)
    expect(css).toMatch(/\.folder-peek-panel,[\s\S]*?padding:\s*0 1px 0/)
    ...
    expect(css).toMatch(/\.folder-peek-panel dl > div,[\s\S]*?min-height:\s*16px/)
    expect(css).toMatch(/\.folder-peek-panel header,[\s\S]*?min-height:\s*13px/)
    expect(css).toMatch(/\.peek-tabs\s*\{[\s\S]*?min-height:\s*13px/)
    ...
    expect(folderView).toMatch(/data-peek-density="capture-pass4"/)
    expect(folderView).toMatch(/\.folder-peek-panel\s*\{[\s\S]*?padding:\s*0 1px 0/)
    expect(folderView).toMatch(/\.folder-peek-panel\s*\{[\s\S]*?font-size:\s*11px/)
    expect(folderView).toMatch(/\.folder-peek-panel header\s*\{[\s\S]*?min-height:\s*13px/)
    ...
    expect(syncView).toMatch(/data-peek-density="capture-pass4"/)
    expect(syncView).toMatch(/\.folder-sync-peek-panel\s*\{[\s\S]*?padding:\s*0 1px 0/)
    expect(syncView).toMatch(/\.folder-sync-peek-panel header\s*\{[\s\S]*?min-height:\s*13px/)

    expect(mergeView).toMatch(/data-testid="folder-merge-peek-tabs"/)
    expect(mergeView).toMatch(/data-testid="folder-merge-peek-dual"/)
    expect(mergeView).toMatch(/data-peek-density="capture-pass4"/)
    expect(mergeView).toMatch(/\.folder-merge-peek-panel\s*\{[\s\S]*?padding:\s*0 1px 0/)
    expect(mergeView).toMatch(/\.folder-merge-peek-panel header\s*\{[\s\S]*?min-height:\s*13px/)
  })
})
