...
describe('peek panel densify', () => {
  it('keeps Folder/Sync/Merge peek panels dense with tab chrome', () => {
    ...
    expect(css).toMatch(/\.peek-tab\s*\{[\s\S]*?height:\s*16px/)
    expect(css).toMatch(/\.peek-tab\s*\{[\s\S]*?font-size:\s*11px/)
    expect(css).toMatch(/\.folder-peek-panel,[\s\S]*?padding:\s*1px 3px 2px/)
    expect(css).toMatch(/\.peek-dual-columns\s*\{/)
    expect(css).toMatch(/grid-template-columns:\s*minmax\(0, 1fr\) minmax\(0, 1fr\)/)
    expect(css).toMatch(/\.peek-dual-columns\s*\{[\s\S]*?gap:\s*2px/)
    expect(css).toMatch(/\.folder-peek-panel dl > div,[\s\S]*?min-height:\s*22px/)
    ...
    expect(folderView).toMatch(/\.folder-peek-panel\s*\{[\s\S]*?padding:\s*1px 3px 2px/)
    expect(folderView).toMatch(/\.folder-peek-panel\s*\{[\s\S]*?font-size:\s*11px/)
    expect(folderView).toMatch(/\.folder-peek-panel header\s*\{[\s\S]*?min-height:\s*16px/)

    expect(syncView).toMatch(/data-testid="folder-sync-peek-tabs"/)
    expect(syncView).toMatch(/data-testid="folder-sync-peek-dual"/)
    expect(syncView).toMatch(/\.folder-sync-peek-panel\s*\{[\s\S]*?padding:\s*1px 3px 2px/)
    expect(syncView).toMatch(/\.folder-sync-peek-panel header\s*\{[\s\S]*?min-height:\s*16px/)

    expect(mergeView).toMatch(/data-testid="folder-merge-peek-tabs"/)
    expect(mergeView).toMatch(/data-testid="folder-merge-peek-dual"/)
    expect(mergeView).toMatch(/\.folder-merge-peek-panel\s*\{[\s\S]*?padding:\s*1px 3px 2px/)
    expect(mergeView).toMatch(/\.folder-merge-peek-panel header\s*\{[\s\S]*?min-height:\s*16px/)
  })
})
