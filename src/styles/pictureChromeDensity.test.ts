...
describe('picture compare chrome density', () => {
  it('keeps Picture Compare path/status/toolbar/footer chrome at capture CSS scale', () => {
    ...
    expect(layout).toMatch(/data-chrome-kind='picture-session'[\s\S]*?height:\s*20px/)
  ...
  it('keeps Picture Meta/Blend/Tol chrome on capture band', () => {
    ...
    expect(css).toMatch(
      /\.picture-compare-view \.metadata-grid-heading,\s*\.picture-compare-view \.metadata-cell\s*\{[\s\S]*?padding:\s*1px 5px/,
    )
    expect(css).toMatch(
