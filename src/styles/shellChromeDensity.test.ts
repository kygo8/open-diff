...
describe('shell chrome density', () => {
  ...
  it('densifies multi-tab title/menu/tab strip toward captures', () => {
    ...
    expect(source).toMatch(/\.tab-chip button\s*\{[\s\S]*?height:\s*13px/)
  ...
  it('keeps WorkbenchShell session toolbar/content frame denser', () => {
    ...
    expect(mainCss).toMatch(/\.workbench-titlebar\s*\{[\s\S]*?height:\s*18px/)
  ...
  it('marks WorkbenchShell frame densify pass 8', () => {
    expect(workbench).toMatch(/data-shell-density="capture-pass8"/)
  })
})
