...
describe('folder sync/merge chrome density', () => {
  it('keeps Sync/Merge path/filters/actions/status chrome dense toward Folder Compare', () => {
    ...
    expect(css).toMatch(
      /\.folder-sync-view \.sync-setting-actions \.n-button[\s\S]*?height:\s*20px/,
    )
    ...
    expect(css).toMatch(/\.folder-merge-view \.merge-actions \.n-button[\s\S]*?height:\s*20px/)
  ...
  it('keeps Sync/Merge plan/preview/action chrome dense one more notch vs Folder Compare band', () => {
    expect(css).toMatch(/\.folder-sync-view \.sync-preview-row span[\s\S]*?padding:\s*2px 6px/)
    expect(css).toMatch(/\.folder-sync-view \.sync-preview-row select[\s\S]*?height:\s*20px/)
    expect(css).toMatch(/\.folder-sync-view \.sync-chrome-panel\s*\{[\s\S]*?min-height:\s*20px/)
    expect(css).toMatch(/\.folder-merge-view \.merge-plan-row span[\s\S]*?padding:\s*2px 6px/)
    expect(css).toMatch(/\.folder-merge-view \.merge-actions \.n-button[\s\S]*?height:\s*20px/)
    expect(css).toMatch(/\.folder-merge-view \.conflict-panel li\s*\{[\s\S]*?padding:\s*4px 6px/)

    expect(syncView).toMatch(/\.sync-preview-row span[\s\S]*?padding:\s*2px 6px/)
    expect(syncView).toMatch(/\.sync-preview-row select\s*\{[\s\S]*?height:\s*20px/)
    expect(syncView).toMatch(/\.sync-chrome-panel\s*\{[\s\S]*?min-height:\s*20px/)
    expect(syncView).toMatch(/\.sync-preview,\s*\.sync-run-status\s*\{[\s\S]*?padding:\s*2px 4px/)

    expect(mergeView).toMatch(/\.merge-plan-row span[\s\S]*?padding:\s*2px 6px/)
    expect(mergeView).toMatch(/\.merge-chrome-panel\s*\{[\s\S]*?min-height:\s*20px/)
    expect(mergeView).toMatch(/\.merge-chrome-panel button\s*\{[\s\S]*?height:\s*20px/)
