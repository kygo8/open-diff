...
describe('folder sync/merge chrome density', () => {
  it('keeps Sync/Merge path/filters/actions/status chrome dense toward Folder Compare', () => {
    expect(css).toMatch(/\.folder-sync-view \.sync-settings\s*\{[\s\S]*?padding:\s*4px 6px/)
    expect(css).toMatch(/\.folder-sync-view \.sync-settings input\s*,[\s\S]*?height:\s*22px/)
    expect(css).toMatch(
      /\.folder-sync-view \.sync-setting-actions \.n-button[\s\S]*?height:\s*22px/,
    )
    expect(css).toMatch(/\.folder-merge-view \.merge-paths\s*\{[\s\S]*?padding:\s*4px 6px/)
    expect(css).toMatch(/\.folder-merge-view \.merge-path-footer\s*\{[\s\S]*?font-size:\s*10px/)
    expect(css).toMatch(/\.folder-merge-view \.merge-actions \.n-button[\s\S]*?height:\s*22px/)

    expect(syncView).toMatch(/\.sync-settings\s*\{[\s\S]*?padding:\s*4px 6px/)
    ...
    expect(mergeView).toMatch(/\.merge-paths\s*\{[\s\S]*?padding:\s*4px 6px/)
    expect(mergeView).toMatch(/\.merge-path-footer\s*\{[\s\S]*?font-size:\s*10px/)
