...
describe('text compare chrome density', () => {
  it('keeps Text Compare path/status/editor chrome dense toward capture', () => {
    ...
    expect(css).toMatch(/\.text-compare-view \.bc-path-row\s*\{[\s\S]*?min-height:\s*22px/)
    expect(css).toMatch(/\.text-compare-view \.bc-path-row input\s*\{[\s\S]*?height:\s*18px/)
    ...
    expect(textView).toMatch(/\.toolbar-button\s*\{[\s\S]*?height:\s*20px/)
  ...
  it('keeps Text Compare toolbar/gutter/header/wrap chrome dense one more notch', () => {
    expect(css).toMatch(/\.text-compare-view \.toolbar-button\s*\{[\s\S]*?height:\s*20px/)
    ...
    expect(textView).toMatch(/\.compare-toolbar\s*\{[\s\S]*?gap:\s*3px/)
