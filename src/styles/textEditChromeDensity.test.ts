...
describe('text edit chrome density', () => {
  it('keeps Text Edit path/toolbar/editor/status chrome dense toward capture', () => {
    ...
    expect(css).toMatch(/\.text-edit-view \.path-input[\s\S]*?height:\s*16px/)
    ...
    expect(editView).toMatch(/\.path-input[\s\S]*?height:\s*16px/)
  ...
  it('keeps Text Edit chrome dense one more notch vs Text Compare band', () => {
    ...
    expect(css).toMatch(/\.text-edit-view \.find-input[\s\S]*?height:\s*16px/)
    ...
    expect(editView).toMatch(/\.find-input[\s\S]*?height:\s*16px/)
